...
$(document).ready(function () {
    ...
    $.ajax({
        type: "GET",
        url: "yml/conference.yml",
        success: function (data) {
            var conference = jsyaml.load(data);
            if (!conference){
                var coming = '<h2 style="text-align: center; margin: 100px auto">Coming Soon</h2>';
                $('.conference').append(coming);
            }else {
                generateYearFolder(conference);
                conference.forEach(function (conference) {

                    var title = conference.title;
                    var date = conference.date;
                    var time = conference.time;
                    var poster = conference.poster;
                    var detail = conference.detail;
                    var venue = conference.venue;
                    //changeable variable
                    var meeting = conference.meeting;
                    var video = conference.video;
                    var photo = conference.photo;
